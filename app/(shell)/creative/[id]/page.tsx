...
import { getCreative, renderedMedia, renderedSlideMedia, templateLabel } from "@/lib/creatives";
...
const BTN_PRIMARY =
  "inline-flex h-10 items-center gap-1.5 rounded-xl bg-foreground px-4 text-body font-medium text-background transition-colors hover:bg-action-hover";
const BTN_SECONDARY =
    ...
    <div className="mx-auto max-w-6xl px-6 py-10">
      ...
      <header className="mt-5 pb-6">
        <div className="flex flex-wrap items-center gap-2">
          <Mono className="text-t3">{templateLabel(brief, tpl?.label)}</Mono>
          {isVideo ? (
            <Mono className="rounded-full bg-subtle px-2 py-0.5 text-t3">
                ...
                <div className="flex items-center justify-center bg-subtle p-4">
                  {media?.ext === "mp4" ? (
                    <video
                      ...
                      className="max-h-[420px] w-auto max-w-full rounded-xl"
                      controls
                      muted
                      loop
                      playsInline
